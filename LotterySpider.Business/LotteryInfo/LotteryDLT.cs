...
{
    public class LotteryDLT : ILotteryFactory
    {
        ...
        public List<Dictionary<int, int>> GetLotteryDataMaxFrequency(List<LotteryBaseInfo> baseInfoList)
        {
            ...
            if (baseInfoList != null)
            {
                ...
                maxRedDict = redDict.OrderByDescending(p => p.Value).Take(5).OrderBy(p => p.Key).ToDictionary(p => p.Key, p => p.Value);
                maxBlueDict = blueDict.OrderByDescending(p => p.Value).Take(2).OrderBy(p => p.Key).ToDictionary(p => p.Key, p => p.Value);
            }
        ...
        public List<Dictionary<int, int>> GetLotteryDataMinFrequency(List<LotteryBaseInfo> baseInfoList)
        {
            ...
            if (baseInfoList != null)
            {
                ...
                minRedDict = redDict.OrderBy(p => p.Value).Take(5).OrderBy(p => p.Key).ToDictionary(p => p.Key, p => p.Value);
                minBlueDict = blueDict.OrderBy(p => p.Value).Take(2).OrderBy(p => p.Key).ToDictionary(p => p.Key, p => p.Value);
            }
